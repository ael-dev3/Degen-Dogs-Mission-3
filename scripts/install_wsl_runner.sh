...
if [[ "$enable_now" == "1" ]]; then
  ...
  origin_url="$(run_as_runner_runtime git -C "$repo_dir" remote get-url origin)"
  ...
  [[ -z "$(run_as_runner_runtime git -C "$repo_dir" config --local --get-all remote.origin.pushurl || true)" ]] || \
    fail "origin must not define a separate pushurl"
  push_urls="$(run_as_runner_runtime git -C "$repo_dir" remote get-url --push --all origin)"
  ...
  [[ "$(run_as_runner_runtime git -C "$repo_dir" config --local --get core.sshCommand)" == "$expected_ssh_command" ]] || \
