...
const restoreAuctionRows=()=>{if(!auctionBody)return;loadUnified().then(records=>{setAuctionRows(records.slice(0,10),'Latest 10 archive records');updateCounts();}).catch(()=>{auctionBody.replaceChildren(...defaultRows.map(row=>row.cloneNode(true)));if(auctionTotal){auctionTotal.dataset.total=String(defaultRows.length);auctionTotal.textContent=`${defaultRows.length} rows`;}updateCounts();});};
